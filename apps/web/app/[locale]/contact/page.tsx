...
export async function generateMetadata({
  ...
  const cmsPage = await getCorePageContent("contact", typedLocale)
  const title = cmsPage?.metaTitle ?? cmsPage?.title ?? dict.contact.contactUs
  const description = cmsPage?.metaDescription ?? cmsPage?.heroDescription ?? dict.contact.metaDescription

  return {
    title,
    description,
    alternates: localizedHreflangAlternates("/contact/", typedLocale),
    openGraph: {
      title,
      description,
      url: localizedHreflangAlternates("/contact/", typedLocale).canonical,
    },
  }
}
...
export default async function ContactPage({ params }: Props) {
  ...
  const typedLocale = locale as Locale
  const dict = getDictionary(typedLocale)
  const info = getBusinessInfo(typedLocale)
  const cmsPage = await getCorePageContent("contact", typedLocale)
  ...
  return (
    <>
      <PageHero
        ...
        title={cmsPage?.title ?? dict.contact.contactUs}
        description={cmsPage?.heroDescription ?? dict.contact.heroDescription}
      />
