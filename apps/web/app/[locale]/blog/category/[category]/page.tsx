...
import { getAllCategoriesWithCms, getBlogPostsByCategoryWithCms } from "@/lib/data/blog-posts"
...
export async function generateStaticParams() {
  const params = await Promise.all(
    nonDefaultLocales.map(async (locale) =>
      (await getAllCategoriesWithCms(locale)).map((category) => ({ locale, category: category.slug })),
    ),
  )
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const categories = await getAllCategoriesWithCms(typedLocale)
...
export default async function BlogCategoryPage({ params }: Props) {
  ...
  const categoryPosts = await getBlogPostsByCategoryWithCms(categorySlug, typedLocale)
