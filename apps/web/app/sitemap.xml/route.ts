...
import { blogPosts, getAllBlogPostsWithCms, getAllCategories, getAllCategoriesWithCms, type BlogPost } from "@/lib/data/blog-posts"
...
export function buildEntries(posts: Pick<BlogPost, "slug">[] = blogPosts, categories = getAllCategories()): SitemapEntry[] {
  const staticPaths = [
...
export async function buildEntriesWithCms(): Promise<SitemapEntry[]> {
  const [posts, categories] = await Promise.all([
    getAllBlogPostsWithCms("en"),
    getAllCategoriesWithCms("en"),
  ])

  return buildEntries(posts, categories)
}
...
export async function GET() {
  const entries = await buildEntriesWithCms()
