        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <ul className="space-y-3">
            {relatedServices.map((service) => (
              <li key={service.slug}>
                <Link href={service.canonicalPath} className="text-sm text-muted-foreground hover:text-[#4AA69D]">
                  {service.name}
                </Link>
              </li>
            ))}
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <ul className="space-y-4">
            {relatedPosts.map((post) => (
              <li key={post.slug}>
                <Link href={`/blog/${post.slug}/`} className="block text-sm font-medium text-foreground hover:text-[#4AA69D]">
                  {post.title}
                </Link>
